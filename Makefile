...
run_local_heroku: $(HEROKU) .once-go-get
...
build_cloud: $(CLOUD_APP)

$(APP): $(GO) .once-go-get $(SRC)
	$(GO) build -o $@ $(SRC)

$(CLOUD_APP): $(GO) .once-go-get $(SRC)
...
# AWS Lambda
deploy_lambda: $(AWS) .once-init-aws $(CLOUD_APP)
	# nyi

# Amazon EC2 Container Service
deploy_ecs: $(AWS) .once-init-aws $(CLOUD_APP)
	# nyi

# Google App Engine
deploy_gae: $(GCLOUD) deps_gae .once-init-gcloud $(CLOUD_APP)
	$(GCLOUD) app deploy gae/app.yaml

# Google Container Engine
deploy_gce: $(GCLOUD) deps_gce .once-init-gcloud $(CLOUD_APP)
	# nyi

# Google Cloud Functions
deploy_gcf:
	@echo "error: Google Cloud Functions does not run go (yet)" 1>&2
	@exit 1

# Heroku Dynos
deploy_heroku: $(HEROKU) .once-init-heroku $(CLOUD_APP)
	# nyi

# DigitalOcean Droplet (Debian)
deploy_digitalocean: $(DOCTL) .once-init-digitalocean $(CLOUD_APP)
	# nyi
...
# TJ's Up
deploy_up: $(UP) .once-init-up $(CLOUD_APP)
	# nyi
